...
record Psh : Set₁ where
  ...
  field
    ...
    wk-pres-trans : ∀ {x : Fam Γ} (w : Γ ⊆ Γ') (w' : Γ' ⊆ Γ'') → wk (⊆-trans w w') x ≋ wk w' (wk w x)
...
open Psh public
  ...
  renaming
    ...
    ; ≋-sym       to ≋-sym[_]
    ; ≋-trans     to ≋-trans[_]
    ; ≋-reflexive to ≋-reflexive[_]
    ; wk          to wk[_]
    ; wk-pres-≋   to wk-pres-≋[_]
...
record _≈̇_ (φ ψ : 𝒫 →̇ 𝒬) : Set where -- type \~~ \^.
  ...
  apply-sq : ∀ {p p' : 𝒫 ₀ Γ} → p ≋[ 𝒫 ] p' → φ .apply p ≋[ 𝒬 ] ψ .apply p' -- XXX: name
...
module _ {𝒫 𝒬 : Psh} where
  abstract
    ...
    ≈̇-refl = record { proof = λ {_} _ → ≋-refl[ 𝒬 ] }

    ≈̇-sym : Symmetric {A = 𝒫 →̇ 𝒬} _≈̇_
    ≈̇-sym φ≋φ' = record { proof = λ {_} p → ≋-sym[ 𝒬 ] (φ≋φ' ._≈̇_.proof p) }

    ≈̇-trans : Transitive {A = 𝒫 →̇ 𝒬} _≈̇_
    ≈̇-trans φ≋ψ ψ≋ω = record { proof = λ {_} p → ≋-trans[ 𝒬 ] (φ≋ψ ._≈̇_.proof p) (ψ≋ω ._≈̇_.proof p) }
...
abstract
  ...
  ∘-assoc {_} {ℛ} ω ψ φ = record { proof = λ p → ≋-refl[ ℛ ] }

id'[_] : (𝒫 : Psh) → 𝒫 →̇ 𝒫
id'[_] 𝒫 = record
  { fun     = λ p → p
  ; pres-≋  = λ p≋p' → p≋p'
  ; natural = λ _ _ → ≋-refl[ 𝒫 ]
  }
...
abstract
  ...
  id'-unit-left 𝒬 _ = record { proof = λ p → ≋-refl[ 𝒬 ] }

  id'-unit-right : ∀ (𝒫 : Psh) {𝒬 : Psh} (φ : 𝒫 →̇ 𝒬) → φ ∘ id'[ 𝒫 ] ≈̇ φ
  id'-unit-right _ {𝒬} _ = record { proof = λ p → ≋-refl[ 𝒬 ] }
