...
module Semantics.Presheaf.CartesianClosure
  ...
  (⊆-refl            : ∀ {Γ : C} → Γ ⊆ Γ)
  (⊆-refl-unit-left  : ∀ {Γ Γ' : C} (w : Γ ⊆ Γ') → ⊆-trans ⊆-refl w ≡ w)
  (⊆-refl-unit-right : ∀ {Γ Γ' : C} (w : Γ ⊆ Γ') → ⊆-trans w ⊆-refl ≡ w)
  where
...
unit' = record
  { fun     = λ _r → tt
  ; pres-≋  = λ {Γ} _p≋p' → ≋-refl[ Unit' ] {Γ}
  ; natural = λ {_Γ} {Δ} _w _r → ≋-refl[ Unit' ] {Δ}
  }
...
module _ (𝒫 𝒬 : Psh) where
  ...
  private
    ...
    ≋-equiv _Γ = record
      { refl  = proof (≋-refl[ 𝒫 ] , ≋-refl[ 𝒬 ])
      ; sym   = λ x≋y → let proof (p≋p' , q≋q') = x≋y in proof (≋-sym[ 𝒫 ] p≋p' , ≋-sym[ 𝒬 ] q≋q')
      ; trans = λ x≋y y≋z → let proof (p≋p' , q≋q') = x≋y; proof (p'≋p'' , q'≋q'') = y≋z in proof (≋-trans[ 𝒫 ] p≋p' p'≋p'' , ≋-trans[ 𝒬 ] q≋q' q'≋q'')
      }
  ...
  _×'_ = record
    ...
    ; wk-pres-≋     = λ w x≋y → let proof (p≋p' , q≋q') = x≋y in proof (wk-pres-≋[ 𝒫 ] w p≋p' , wk-pres-≋[ 𝒬 ] w q≋q')
    ; wk-pres-refl  = {!!}
    ; wk-pres-trans = {!!}
    }
...
module _ {𝒫 𝒬 : Psh} where
  ...
  π₁' = record
    ...
    ; natural = λ _w x → let elem (_p , _q) = x in ≋-refl[ 𝒫 ]
    }
  ...
  π₂' = record
    ...
    ; natural = λ _w x → let elem (_p , _q) = x in ≋-refl[ 𝒬 ]
    }
  ...
  abstract
    ...
    ×'-beta-left {_t} _u = record { proof = λ _r → ≋-refl[ 𝒫 ] }

    ×'-beta-right : ∀ (t : ℛ →̇ 𝒫) {u : ℛ →̇ 𝒬} → snd' (pr' t u) ≈̇ u
    ×'-beta-right t {_u} = record { proof = λ _r → ≋-refl[ 𝒬 ] }

    ×'-eta : t ≈̇ pr' (fst' t) (snd' t)
    ×'-eta = record { proof = λ _r → ≋-refl[ 𝒫 ×' 𝒬 ] }
...
abstract
  ...
  ×'-map-pres-id {𝒫} {𝒬} = record { proof = λ _x → ≋-refl[ 𝒫 ×' 𝒬 ] }
...
module _ (𝒫 𝒬 : Psh) where
  ...
  private
    ...
    ⇒'-≋-equiv _Γ = record
      { refl  = proof (λ _w _p → ≋-refl[ 𝒬 ])
      ; sym   = λ {f} {g} f≋g → proof (λ w p → ≋-sym[ 𝒬 ] (f≋g .pw w p))
      ; trans = λ {f} {g} {h} f≋g g≋h → proof (λ w p → ≋-trans[ 𝒬 ] (f≋g .pw w p) (g≋h .pw w p))
      }
...
module _ {𝒫 𝒬 : Psh} where
  ...
  app' {ℛ} t u = record
    ...
    ; natural = λ w r → let open EqReasoning ≋[ 𝒬 ]-setoid in begin
        ...
        t .apply r .apply (⊆-trans ⊆-refl w) (u .apply (wk[ ℛ ] w r))       ≡⟨ cong (λ hole → t .apply r .apply hole (u .apply (wk[ ℛ ] w r))) (⊆-refl-unit-left w) ⟩
        t .apply r .apply w                  (u .apply (wk[ ℛ ] w r))       ≡˘⟨ cong (λ hole → t .apply r .apply hole (u .apply (wk[ ℛ ] w r))) (⊆-refl-unit-right w) ⟩
  ...
  abstract
    ...
    app'-nat _t _u _s = record { proof = λ _r' → ≋-refl[ 𝒬 ] }

lam' : (t : ℛ ×' 𝒫 →̇ 𝒬) → ℛ →̇ 𝒫 ⇒' 𝒬
lam' {ℛ} t = record
  { fun     = λ r → elem (λ w p → t .apply (elem (wk[ ℛ ] w r , p))) (λ w p≋p' → t .apply-≋ (proof (≋-refl[ ℛ ] , p≋p'))) (λ w w' p → {!!})
  ; pres-≋  = {!!}
  ; natural = {!!} }
