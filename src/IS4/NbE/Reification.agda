...
reify-natural (a ⇒ b) x@(f , _f-natural) w = let open ≡-Reasoning in begin
  reify (a ⇒ b) (wk[ evalTy (a ⇒ b) ] w x)                                                               ≡⟨⟩
  lam (reify b (f (w ∙ fresh {a = a}) (reflect a var0)))                                                 ≡⟨ {!!} ⟩
  lam (reify b (f (fresh {a = a} ∙ keep {a = a} w) (reflect a (wkNe (keep {a = a} w) var0))))            ≡⟨ {!!} ⟩
  lam (reify b (f (fresh {a = a} ∙ keep {a = a} w) (wk[ evalTy a ] (keep {a = a} w) (reflect a var0))))  ≡⟨ {!!} ⟩
  lam (reify b (wk[ evalTy b ] (keep {a = a} w) (f (fresh {a = a}) (reflect a var0))))                   ≡⟨ {!!} ⟩
  lam (wkNf (keep {a = a} w) (reify b (f (fresh {a = a}) (reflect a var0))))                             ≡⟨⟩
  wkNf w (reify (a ⇒ b) x)                                                                               ∎
reify-natural (◻ a) x w = {!!}
