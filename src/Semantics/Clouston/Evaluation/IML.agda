...
module Eval (N : Ty') where
  ...
  abstract
    ...
    evalWk-pres-id Γ@(Γ' `, a) = let open EqReasoning (Sub'-setoid Γ Γ) in begin
      evalWk (keep {a = a} idWk[ Γ' ])          ≈⟨ ×'-map-pres-≈̇-left (evalWk-pres-id Γ') id'[ evalTy a ] ⟩
    ...
    evalWk-pres-∘-π₁ : evalWk (drop {a = a} w) ≈̇ evalWk w ∘ π₁'[ evalTy a ]
    evalWk-pres-∘-π₁ = ≈̇-refl

    evalWk-pres-×-map-id : evalWk (keep {a = a} w) ≈̇ evalWk w ×'-map id'[ evalTy a ]
  ...
  module _ {a : Ty} where
    abstract
      ...
      evalVar-pres-∘ (keep {Δ = Δ} {b} w) (su {Γ} {a} {b} n) = let open EqReasoning (Tm'-setoid (Δ `, b) a) in begin
        ...
        evalVar n ∘ π₁'[ evalTy b ] ∘ evalWk (keep {Δ = Δ} {b} w)  ≈˘⟨ ∘-assoc (evalVar n) π₁'[ evalTy b ] (evalWk (keep {Δ = Δ} {b} w)) ⟩
        evalVar (su {Γ} {a} {b} n) ∘ evalWk (keep {Δ = Δ} {b} w)   ∎
